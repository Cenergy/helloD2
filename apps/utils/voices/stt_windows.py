...
if plat[0] == '32bit':
    dll = cdll.LoadLibrary(BASEPATH + '\\x86\msc.dll')
else:
    dll = cdll.LoadLibrary(BASEPATH + '\\x64\msc_x64.dll')

#  # 调用动态链接库
# dll = cdll.LoadLibrary(Lib_PATH)
 #登录参数，apppid一定要和你的下载SDK对应
login_params = b"appid = 5b3483ed, work_dir = ."
...
class Msp:
    ...
    def isr(self, audiofile, session_begin_params):
        ...
        print(laststr)
        ret = dll.QISRSessionEnd(sessionID, '\n')
        # print('end ret: ', ret)
        return laststr
